...
***************************/
// Pull CS pin low
static inline void MAX11200_CS_Low(MAX11200_ADC *adc){
    HAL_GPIO_WritePin(adc->CS_GPIO_Port, adc->CS_GPIO_Pin, GPIO_PIN_RESET);
}

// Pull CS pin high
static inline void MAX11200_CS_High(MAX11200_ADC *adc){
    HAL_GPIO_WritePin(adc->CS_GPIO_Port, adc->CS_GPIO_Pin, GPIO_PIN_SET);
}
...
static void MAX11200_WriteReg8(MAX11200_ADC *adc, uint8_t regAddr, uint8_t data){
    uint8_t cmd = MAX11200_BuildWriteCmd(regAddr);

    osMutexAcquire(spiMutex, osWaitForever);
    MAX11200_CS_Low(adc);
    HAL_SPI_Transmit(adc->hspi, &cmd, 1, 100);
    HAL_SPI_Transmit(adc->hspi, &data, 1, 100);
    MAX11200_CS_High(adc);
    osMutexRelease(spiMutex);
}

// Read an 8-bit value from a specified register
static uint8_t MAX11200_ReadReg8(MAX11200_ADC *adc, uint8_t regAddr){
    ...
    MAX11200_CS_Low(adc);
    HAL_SPI_Transmit(adc->hspi, &cmd, 1, 100);
    HAL_SPI_Receive(adc->hspi, &rxByte, 1, 100);
    MAX11200_CS_High(adc);
    osMutexRelease(spiMutex);
...
static int32_t MAX11200_ReadReg24(MAX11200_ADC *adc, uint8_t regAddr){
    uint8_t cmd = MAX11200_BuildReadCmd(regAddr);
    uint8_t buf[3] = {0};
    int32_t result;
    ...
    MAX11200_CS_Low(adc);
    HAL_SPI_Transmit(adc->hspi, &cmd, 1, 100);
    HAL_SPI_Receive(adc->hspi, buf, 3, 100);
    MAX11200_CS_High(adc);
    osMutexRelease(spiMutex);
...
int32_t MAX11200_ReadData24(MAX11200_ADC *adc){
    return MAX11200_ReadReg24(adc, MAX11200_DATA_REG);
}

// Read control and status registers into MAX11200_CtrlStat_Regs
static void MAX11200_ReadCtrlStatRegs(MAX11200_ADC *adc)
{
    MAX11200_CtrlStat_Regs.ctrl1 = MAX11200_ReadReg8(adc, MAX11200_CTRL1_REG);
    MAX11200_CtrlStat_Regs.ctrl2 = MAX11200_ReadReg8(adc, MAX11200_CTRL2_REG);
    MAX11200_CtrlStat_Regs.ctrl3 = MAX11200_ReadReg8(adc, MAX11200_CTRL3_REG);
    MAX11200_CtrlStat_Regs.stat1 = MAX11200_ReadReg8(adc, MAX11200_STAT1_REG);
}
...
void MAX11200_Read_Config(MAX11200_ADC *adc, MAX11200_Config_Data *config)
{
    ...
    uint8_t ctrl1 = MAX11200_ReadReg8(adc, MAX11200_CTRL1_REG);
...
void MAX11200_Write_Config(MAX11200_ADC *adc, MAX11200_Config_Data *config)
{
    ...
    MAX11200_WriteReg8(adc, MAX11200_CTRL1_REG, ctrl1);
...
{
    uint8_t regval = MAX11200_ReadReg8(adc, MAX11200_STAT1_REG);
...
{
    uint8_t stat = MAX11200_Read_Stat(adc);
...
{
    MAX11200_Start_Conversion(adc, RATE);
    while(!MAX11200_Conversion_Ready(adc)){
        osDelay(pdMS_TO_TICKS(1));
    }

    uint32_t val = MAX11200_ReadReg24(adc, MAX11200_DATA_REG);
    return val;
}
...
void MAX11200_Self_Calibration(MAX11200_ADC *adc, uint32_t *calib_offset, uint32_t *calib_gain)
{
    ...
    MAX11200_WriteReg8(adc, MAX11200_CTRL3_REG, ctrl3);
    ...
    *calib_offset = MAX11200_ReadReg24(adc, MAX11200_SCOC_REG);
    *calib_gain = MAX11200_ReadReg24(adc, MAX11200_SCGC_REG);
}
